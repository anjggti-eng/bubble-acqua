...
import { LayoutDashboard, Calendar, Scissors, Image, User } from 'lucide-react';
import { usePendingCount } from '@/hooks/usePendingCount';

const navItems = [
  { path: '/minha-loja', icon: LayoutDashboard, label: 'Dashboard' },
  { path: '/agendamentos-loja', icon: Calendar, label: 'Agenda' },
...
export default function OwnerNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-lg border-t border-primary/20">
      <div className="flex items-center justify-around h-16 max-w-lg mx-auto px-2 safe-bottom">
        {navItems.map(({ path, icon: Icon, label }) => {
          const showBadge = path === '/agendamentos-loja' && pendingCount > 0;
          return (
            <NavLink
              key={path}
              to={path}
              className={({ isActive }) =>
                `flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-lg transition-all duration-200 relative ${
