...
import { Home, Search, Calendar, Store, User } from 'lucide-react';
...
const navItems = [
  ...
  { path: '/minha-loja', icon: Store, label: 'Minha Loja' },
  { path: '/perfil', icon: User, label: 'Perfil' },
];

// Sub-pages that should hide the BottomNav
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-lg border-t border-border">
      <div className="flex items-center justify-around h-16 max-w-lg mx-auto px-2 safe-bottom">
        {navItems.map(({ path, icon: Icon, label }) => {
          const showBadge = (path === '/agendamentos' || path === '/minha-loja') && pendingCount > 0;
          return (
            <NavLink
              key={path}
              to={path}
